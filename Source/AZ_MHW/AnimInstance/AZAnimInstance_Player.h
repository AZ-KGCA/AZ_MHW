...
private:
	 ...
	 * 모든 클라이언트에서 캐릭터는 시작위치, 시작방향, 시작시간, 실행 애니메이션이 동일하면 동일한 결과를 가지도록 한다.
...
public:
	...
	UPROPERTY(BlueprintReadWrite)
	bool is_hold_rotation_;//현재 애니메이션이 강제 회전보간(목표 방향으로)인지
	UPROPERTY(BlueprintReadWrite)
	...
	UFUNCTION(BlueprintCallable,Category="AZAnimSystem")
	void SetAnimPlayRate(int32 play_rate = 15000);
